...
<div class="left-sidebar">
    <div class="left-category">
        <div class="list-group" style="text-align: center";>
            @if(!request()->is('business') && !request()->is('business/*') && !request()->is('business/*/*'))
          <!-- Danh mục sản phẩm -->
            ...
            <ul class="menu">
                ...
                <li class="cat-bussiness"><a class="{{ (request('id') == $val['id']) ? 'active-item' : '' }}" href="{{ url('') }}/category/{{ $val['id'] }}" title="">{{ $val->name }}</a>
                    ...
                    <div class="sub-menu">
                        <ul>
                                ...
                                @foreach(app('Home')->getChild($val->id) as $valChild)
                                    <li class="{{ (request('id') == $valChild['id']) ? 'active-item' : '' }}"><a href="{{ url('') }}/category/{{ $valChild['id'] }}" title="">{{ $valChild->name }}</a></li>
                                @endforeach
            ...
            @if(request()->is('business') || request()->is('business/*') || request()->is('business/*/*'))
              <!-- Lĩnh vực kinh doanh -->
                ...
                <a href="{{ url('') }}/business/{{ $val['id'] }}" class="list-group-item list-group-item-action {{ (request('id') == $val['id']) ? 'active-item' : '' }}">{{ $val['name'] }}</a>
                @endforeach
            @endif
        </div>
    </div>  
